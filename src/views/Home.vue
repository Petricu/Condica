            ...
            <v-sheet color="grey lighten-4" class="px-3 pt-3 pb-3">
                ...
                >
                    <v-card class="mx-auto" max-width="500">
                        <v-card-title class="display-4 justify-center">{{ contorDisplay }}</v-card-title>
                        <v-card-actions class="py-5 mx-4">
                            <v-spacer />
                            <v-btn dark large color="red lighten-1" class="px-5">Logout</v-btn>
                        </v-card-actions>
                    </v-card>
                </v-skeleton-loader>
            </v-sheet>
...
export default {
    ...
    mounted() {
        ipcRenderer.on('data', (event: Electron.Event, data: Object) => {
            this.data = data;
