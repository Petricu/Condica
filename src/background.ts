...
ipcMain.on('login-data', (event: Electron.Event, login: string, password: string) => {
    let hasPassed: boolean;
    let url: string = 'http://192.168.1.168:3000/worker';
    let data: Object = {
        login: login,
        password: password,
    };
...
ipcMain.on('signup', (event: Electron.Event, signupData: Object) => {
    const url: string = 'http://192.168.1.168:3000/signup';
    let hasPassed: boolean;
